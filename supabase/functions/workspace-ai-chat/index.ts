...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are Seeksy AI, a context-aware workspace assistant. You help creators and managers be more productive.

Current workspace context:
- Workspace: ${context?.workspace || 'Unknown'}
- Active app: ${context?.activeApp || 'Dashboard'}
- Open panels: ${context?.openPanels?.map((p: any) => p.title).join(', ') || 'None'}
